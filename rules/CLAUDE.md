...
# Skills — routing (catalog: [SKILLS.md](../SKILLS.md))

**Read the catalog before picking a skill.** It lists every installed skill with what it is for, grouped by job. Installed skills live at `~/.claude/skills/<name>/` (global) or `.claude/skills/<name>/` (project); other harnesses use `.agents/skills/`.

**Always on, no invocation needed:** `caveman` (output shape), `i-have-adhd` (output ordering), `avoid-ai-writing` + `humanizer` (word choice on anything that ships). **Reflexes:** `backprop` on any bug or failing test, `prove-it` before claiming a fix works, `find-docs` before answering any library API question.
...
- **Commit identity.** Default author for every repo unless I say otherwise for that repo or session. Before creating or amending a commit, set it:
